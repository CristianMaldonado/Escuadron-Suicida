...
tconfig_memoria* leerConfiguracion() {
	...
	datosMemoria->maximoMarcosPorProceso = config_get_int_value(config,
			"MAXIMO_MARCOS_POR_PROCESO");
	datosMemoria->cantidadMarcos = config_get_int_value(config,
			"CANTIDAD_MARCOS");
	...
	datosMemoria->habilitadaTLB = config_get_string_value(config,
			"TLB_HABILITADA")[0];
	datosMemoria->retardoMemoria = config_get_int_value(config,
			"RETARDO_MEMORIA");
	return datosMemoria;
}
